...
-- 1º.) Elimina todos los productos que contengan la palabra 'Juniperus' =
DELETE FROM producto WHERE nombre LIKE 'Juniperus%';

-- 2º.) Crea una vista de la tabla 'pago' donde se muestre solo la forma de pago y el total a pagar =
CREATE VIEW vista_pagos AS
SELECT pago.forma_pago, pago.total
FROM pago;

-- 3º.) Elimina todos los pedidos que contegan de fecha esperada 'NULL' =
DELETE FROM pedido WHERE fecha_esperada = NULL;

-- 4º.) Actualiza los detalles de los pedidos para que tengan de número de línea el 7 si estos son de frutales =
UPDATE detalle_pedido SET numero_linea = 7 WHERE codigo_producto LIKE 'FR%';

-- 5º.) Intenta eliminar a los empleados por su nombre. ¿Funciona? Si no, explica el por qué de este error =
DELETE nombre FROM empleado;
-- No es posible realizar tal eliminación debido a que ya existen empleados insertados en la tabla. Para que se pueda prodecer a esta acción, tendríamos que eliminar primero cada uno de los empleados insertados.

-- 6º.) Crea una nueva tabla llamada 'relacion_cliente_producto_precio' donde se almacenen el nombre del cliente, el nombre de los productos y la forma de pago =
CREATE TABLE relacion_cliente_producto_precio (
id INT UNSIGNED NOT NULL PRIMARY KEY,
nombre_cliente VARCHAR(50) NOT NULL,
nombre_producto VARCHAR(70) NOT NULL,
total_pago NUMERIC(15,2) NOT NULL,
FOREIGN KEY (nombre_cliente) REFERENCES cliente (nombre_cliente),
FOREIGN KEY (nombre_producto) REFERENCES producto (nombre),
FOREIGN KEY (total_pago) REFERENCES pago (total)
);

-- 7º.) Actualiza el total de los pedidos para que tengan unos 20.55 euros más de cantidad =
UPDATE precio SET total = total + 20.55;